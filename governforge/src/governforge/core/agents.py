...
from governforge.models.governance import AgentActionORM, AgentRunORM, GovernanceAuditEventORM
...
def sweep_stale_runs(session: Session, now: datetime | None = None) -> int:
    """Fail closed when an Agent stops heartbeating before a terminal event."""
    now = now or datetime.now(timezone.utc)
    candidates = list(session.scalars(select(AgentRunORM).where(
        AgentRunORM.status.in_(["running", "waiting_approval"]),
    )))
    timed_out = 0
    for run in candidates:
        ...
        deadline = heartbeat + timedelta(seconds=run.timeout_seconds or 900)
        if deadline >= now:
            continue
        run.status = "timed_out"
        run.completed_at = now
        run.last_error = "AGENT_HEARTBEAT_TIMEOUT"
        session.add(GovernanceAuditEventORM(
            workspace_id=run.workspace_id, actor_id="system", event_type="agent.run.timed_out",
            entity_type="agent_run", entity_id=run.id, trace_id=run.trace_id,
            payload={"deadline": deadline.isoformat(), "timeout_seconds": run.timeout_seconds},
        ))
        timed_out += 1
    return timed_out
