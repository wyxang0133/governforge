...
if test_database_url:
    ...
    os.environ.setdefault("DATABASE_URL", "sqlite:///./devpilot_test.db")
...
def app():
    from devpilot.core.database import Base, engine, init_db
    from devpilot.models import governance, user  # noqa: F401
...
def clean_tasks(app):
    ...
    with engine.begin() as connection:
        for table in ("governance_audit_events", "outbox_events", "agent_evaluations", "agent_approvals", "agent_actions", "agent_runs", "approval_requests", "policy_evaluations", "source_signals", "budget_allocations", "policy_definitions", "knowledge_feedback", "knowledge_escalations", "knowledge_messages", "knowledge_conversations", "knowledge_chunks", "knowledge_articles", "ai_usage_events", "ci_runs", "pull_requests", "repository_grants", "team_memberships", "teams", "webhook_deliveries", "repositories", "github_installations", "auth_sessions", "users"):
