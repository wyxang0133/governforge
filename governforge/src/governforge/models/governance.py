...
class RepositoryORM(Base):
    ...
    auto_merge_enabled: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
    merge_method: Mapped[str] = mapped_column(String(16), default="squash")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
...
class OutboxEventORM(Base):
    ...
    dedupe_key: Mapped[str | None] = mapped_column(String(255), unique=True, index=True)
    payload: Mapped[dict] = mapped_column(JSON, default=dict)
    status: Mapped[str] = mapped_column(String(24), default="pending", index=True)
...
class IntegrationCredentialORM(Base):
    """Revocable machine credential; only a SHA-256 digest is persisted."""
    __tablename__ = "integration_credentials"
    __table_args__ = (UniqueConstraint("workspace_id", "name"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id)
    workspace_id: Mapped[str] = mapped_column(String(36), ForeignKey("workspaces.id"), index=True)
    name: Mapped[str] = mapped_column(String(128))
    provider: Mapped[str] = mapped_column(String(64), default="agent")
    token_prefix: Mapped[str] = mapped_column(String(20), index=True)
    token_hash: Mapped[str] = mapped_column(String(64), unique=True)
    scopes: Mapped[list] = mapped_column(JSON, default=lambda: ["agent:write"])
    active: Mapped[bool] = mapped_column(Boolean, default=True, index=True)
...
class ModelRouteORM(Base):
    """Metadata-only model route. Upstream secrets remain environment references."""
    __tablename__ = "model_routes"
    __table_args__ = (UniqueConstraint("workspace_id", "model_alias", "priority"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id)
    workspace_id: Mapped[str] = mapped_column(String(36), ForeignKey("workspaces.id"), index=True)
    ...
    api_key_env: Mapped[str] = mapped_column(String(128))
...
class CostEventORM(Base):
    """Unified TCO ledger for model, human review, rework and incident cost."""
    ...
    repository_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("repositories.id"), index=True)
    pull_request_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("pull_requests.id"), index=True)
    run_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("agent_runs.id"), index=True)
    trace_id: Mapped[str | None] = mapped_column(String(128), index=True)
    detail: Mapped[dict] = mapped_column(JSON, default=dict)
    occurred_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
