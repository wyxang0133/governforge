...
from devpilot.models.governance import KnowledgeArticleORM, KnowledgeChunkORM
...
def _extractive_answer(question: str, hits: list[RetrievalHit], fallback_reason: str | None = None) -> AnswerResult:
    if not hits:
        ...
        return AnswerResult(answer, 0.1, "devpilot", "extractive-rag", len(question) // 4, len(answer) // 4, 0, fallback_reason)
    ...
    return AnswerResult(answer, round(confidence, 2), "devpilot", "extractive-rag", len(question) // 4, len(answer) // 4, 0, fallback_reason)
...
def answer_question(question: str, hits: list[RetrievalHit], settings: AppSettings) -> AnswerResult:
    if contains_prompt_injection(question):
        ...
        return AnswerResult(answer, 0, "devpilot", "guardrail", len(question) // 4, len(answer) // 4, 0, "prompt_injection")
