...
from governforge.core.policies import get_active_policy, policy_dict
from governforge.core.trace_middleware import get_trace_id
from governforge.core.workspace import get_workspace_id
from governforge.models.governance import GovernanceAuditEventORM, PolicyDefinitionORM, RepositoryORM, TeamORM, WorkspaceORM
...
def export_policies(session: Session = Depends(get_session), workspace_id: str = Depends(get_workspace_id), user: dict = Depends(get_current_user)) -> Response:
    ...
    return Response(yaml.safe_dump({"apiVersion": "governforge.io/v1", "kind": "PolicyBundle", "policies": [{field: getattr(item, field) for field in fields} for item in items]}, allow_unicode=True, sort_keys=False), media_type="application/yaml")
...
def import_policies(request: Request, document: str = Body(..., media_type="application/yaml"), session: Session = Depends(get_session), workspace_id: str = Depends(get_workspace_id), user: dict = Depends(get_current_user)) -> dict:
    ...
    if not isinstance(payload, dict) or payload.get("apiVersion") != "governforge.io/v1" or payload.get("kind") != "PolicyBundle": raise HTTPException(422, "unsupported policy bundle")
