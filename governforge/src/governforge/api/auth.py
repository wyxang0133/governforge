...
from governforge.core.auth import create_access_token, decode_access_token, get_current_user
from governforge.core.database import get_session
from governforge.core.password_hash import hash_password, verify_password
...
from governforge.core.oidc import authorization_request, exchange_and_validate
...
def _set_auth_cookies(response: Response, tokens: TokenResponse) -> None:
    ...
    response.set_cookie("governforge_access", tokens.access_token, httponly=True, secure=secure, samesite="strict", max_age=settings.jwt_expire_minutes * 60, path="/")
    response.set_cookie("governforge_refresh", tokens.refresh_token, httponly=True, secure=secure, samesite="strict", max_age=settings.refresh_token_days * 86400, path="/api/auth")
...
def register(data: Credentials, response: Response, session: Session = Depends(get_session)):
    ...
    if session.get(WorkspaceORM, data.tenant_id) is None:
        session.add(WorkspaceORM(
            id=data.tenant_id,
            name=data.tenant_id,
            slug=data.tenant_id.lower().replace("_", "-")[:64],
        ))
        session.flush()
...
def refresh(response: Response, request: Request, data: RefreshRequest | None = None, session: Session = Depends(get_session)):
    refresh_token = data.refresh_token if data else request.cookies.get("governforge_refresh")
    ...
    if record is None or record.user_id != user.id or record.revoked_at is not None or record.refresh_jti_hash != presented_hash or expires_at <= now:
        ...
        session.commit()
        response.delete_cookie("governforge_access", path="/"); response.delete_cookie("governforge_refresh", path="/api/auth")
...
def logout(response: Response, request: Request, session: Session = Depends(get_session)) -> Response:
    token = request.cookies.get("governforge_refresh")
    if token:
        try:
            ...
            pass
    response.delete_cookie("governforge_access", path="/")
    response.delete_cookie("governforge_refresh", path="/api/auth")
...
def oidc_login() -> RedirectResponse:
    ...
    secure = settings.environment == "production"
    for name, value in (("governforge_oidc_state", state), ("governforge_oidc_nonce", nonce), ("governforge_oidc_verifier", verifier)):
...
def oidc_callback(code: str, state: str, request: Request, session: Session = Depends(get_session)) -> RedirectResponse:
    settings = AppSettings()
    expected_state = request.cookies.get("governforge_oidc_state")
    nonce, verifier = request.cookies.get("governforge_oidc_nonce"), request.cookies.get("governforge_oidc_verifier")
    ...
    for name in ("governforge_oidc_state", "governforge_oidc_nonce", "governforge_oidc_verifier"): response.delete_cookie(name, path="/api/auth/oidc")
    return response
