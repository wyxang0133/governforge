...
from devpilot.core.policy_gate import PolicyEvidence, evaluate_policy
from devpilot.core.scorecard import build_scorecard
from devpilot.core.metrics import POLICY_DECISIONS
from devpilot.core.trace_middleware import get_trace_id
from devpilot.core.auth import get_current_user
from devpilot.core.access import require_repository_access, require_workspace_role, visible_repository_ids
from devpilot.core.budgets import applicable_budgets
from devpilot.core.policies import get_effective_policy
from devpilot.models.governance import AIUsageEventORM, ApprovalRequestORM, CIRunORM, GovernanceAuditEventORM, OutboxEventORM, PolicyEvaluationORM, PullRequestORM, RepositoryORM, SourceSignalORM, WorkspaceORM
...
def decide_approval(approval_id: str, data: ApprovalDecision, session: Session = Depends(get_session), x_workspace_id: str = Depends(get_workspace_id), user: dict = Depends(get_current_user)) -> dict:
    ...
    session.add(OutboxEventORM(workspace_id=x_workspace_id, aggregate_type="pull_request", aggregate_id=item.pull_request_id, event_type="github.check.requested", payload={"evaluation_id": item.evaluation_id, "approval_id": item.id, "decision": effective_decision, "reason": data.reason}))
    session.commit()
...
def list_repositories(session: Session = Depends(get_session), x_workspace_id: str = Depends(get_workspace_id), user: dict = Depends(get_current_user)) -> list[dict]:
    ...
    return [{"id": item.id, "provider": item.provider, "external_id": item.external_id, "full_name": item.full_name, "default_branch": item.default_branch, "installation_id": item.installation_id, "html_url": item.html_url, "active": item.active, "created_at": item.created_at} for item in items]
...
def evaluate_pull_request(pull_request_id: str, request: Request, session: Session = Depends(get_session), x_workspace_id: str = Depends(get_workspace_id), user: dict = Depends(get_current_user)) -> dict:
    ...
    policy = get_effective_policy(session, x_workspace_id, pr.repository_id)
    budget_evidence = applicable_budgets(session, x_workspace_id, pr.repository_id)
    completed = [run for run in runs if run.conclusion]
    ci_passed = bool(completed) and all((run.conclusion or "").lower() in {"success", "passed", "neutral", "skipped"} for run in completed)
    security_runs = [run for run in completed if any(word in run.name.lower() for word in ("security", "sast", "dependency", "secret"))]
    ...
    changed_paths = [path for signal in signals for path in (signal.payload or {}).get("changed_files", [])]
    ...
    evidence = GateRequest(
        pull_request_id=pr.id,
        source_confidence=pr.source_confidence,
        ci_status="passed" if ci_passed else "failed",
        security_status="clean" if security_passed else "unknown",
        test_coverage=max(coverages) if coverages else None,
        estimated_cost_usd=sum(item.cost_usd for item in usage),
        sensitive_files=sensitive_files,
        human_approved=False,
        monthly_budget_exceeded=any(item["hard_limit"] and item["exceeded"] for item in budget_evidence),
        budget_evidence=budget_evidence,
    )
    return evaluate_gate(evidence, request=request, session=session, x_workspace_id=x_workspace_id)
