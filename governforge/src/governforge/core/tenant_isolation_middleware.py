...
from governforge.core.auth import decode_access_token
from governforge.core.tenant_middleware import (
...
# 不需要 JWT 认证的公开路径
PUBLIC_PATHS = {"/health", "/api/health", "/metrics", "/api/auth", "/api/scim", "/api/integrations/github/webhook", "/api/agent-events", "/api/model-gateway/v1/chat/completions"}
...
class TenantIsolationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        token = auth_header[7:] if auth_header.startswith("Bearer ") else request.cookies.get("governforge_access")
        if token:
            try:
