...
def upgrade():
    ...
    op.execute("""
    CREATE OR REPLACE FUNCTION governforge_prevent_audit_mutation() RETURNS trigger AS $$
    ...
    FOR EACH ROW EXECUTE FUNCTION governforge_prevent_audit_mutation();
    """)


def downgrade():
    op.execute("DROP TRIGGER IF EXISTS governance_audit_events_append_only ON governance_audit_events")
    op.execute("DROP FUNCTION IF EXISTS governforge_prevent_audit_mutation()")
