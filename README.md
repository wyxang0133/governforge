...
GovernForge 是面向研发组织的企业 AI 工程平台，包含两条互相打通的业务主线：
...
- 通用 `governforge-agent-hook` CLI，可用于 Codex、Claude Code、Cursor 和自研 Agent 的 hooks。
...
- OpenAI-compatible 统一模型网关：模型别名、优先级路由、有界重试、熔断、硬预算和脱敏 TCO 归因。
- PR 创建/同步及 CI 完成后自动取证和评估；受控自动合并默认关闭且不绕过 GitHub branch protection。
...
Compose 产品名已迁移为 `governforge`。为保留升级前账号、GitHub Installation 和审计证据，本地仍复用旧 PostgreSQL volume 与数据库内部标识；它们是兼容层，不对外暴露为产品品牌。
