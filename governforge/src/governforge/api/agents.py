...
from governforge.models.governance import AgentActionORM, AgentApprovalORM, AgentEvaluationORM, AgentRunORM, GovernanceAuditEventORM, RepositoryORM, WorkspaceORM
...
def decide_approval(approval_id: str, data: ApprovalDecision, request: Request, session: Session = Depends(get_session), workspace_id: str = Depends(get_workspace_id), user: dict = Depends(get_current_user)) -> dict:
    ...
    action = session.get(AgentActionORM, approval.action_id)
    run = session.get(AgentRunORM, approval.run_id)
    if data.decision == "approved" and action.risk_level == "critical" and approval.requested_by == user.get("sub"):
        raise HTTPException(403, "关键风险操作禁止发起人自审批")
