...
from governforge.core.metrics import WEBHOOK_EVENTS
...
from governforge.models.governance import (
    ...
    GitHubInstallationORM, IntegrationCredentialORM, RepositoryORM, SourceSignalORM, WebhookDeliveryORM, WorkspaceORM,
)
...
class AgentCredentialIn(BaseModel):
    ...
    provider: str = Field("agent", min_length=2, max_length=64)
    expires_in_days: int = Field(30, ge=1, le=365)
    scopes: list[str] = Field(default_factory=lambda: ["agent:write"])


@router.post("/agent-credentials", status_code=201)
def create_agent_credential(
    data: AgentCredentialIn,
    request: Request,
    ...
    if session.scalar(select(IntegrationCredentialORM).where(
        IntegrationCredentialORM.workspace_id == workspace_id,
        IntegrationCredentialORM.name == data.name,
    )):
        raise HTTPException(409, "credential name already exists")
    allowed_scopes = {"agent:write", "model:invoke"}
    scopes = sorted(set(data.scopes))
    if not scopes or any(scope not in allowed_scopes for scope in scopes):
        raise HTTPException(422, "scopes must contain agent:write and/or model:invoke")
    raw = ("gf_model_" if scopes == ["model:invoke"] else "gf_agent_") + secrets.token_urlsafe(32)
    item = IntegrationCredentialORM(
    ...
    session.add(GovernanceAuditEventORM(
        ...
        event_type="integration.agent_credential.created", entity_type="integration_credential",
        entity_id=item.id, trace_id=get_trace_id(request),
        payload={"name": item.name, "provider": item.provider, "scopes": scopes, "expires_at": item.expires_at.isoformat()},
    ))
    session.commit()
    return {"id": item.id, "name": item.name, "provider": item.provider, "scopes": scopes, "token": raw, "expires_at": item.expires_at}
...
def list_agent_credentials(
    ...
    if user.get("role") not in {"admin", "owner", "security"}:
        raise HTTPException(403, "admin, owner or security role required")
    items = session.scalars(select(IntegrationCredentialORM).where(
        IntegrationCredentialORM.workspace_id == workspace_id,
    ).order_by(desc(IntegrationCredentialORM.created_at))).all()
    return [{"id": item.id, "name": item.name, "provider": item.provider, "token_prefix": item.token_prefix,
             "scopes": item.scopes, "active": item.active, "expires_at": item.expires_at, "last_used_at": item.last_used_at,
             "created_at": item.created_at} for item in items]


@router.delete("/agent-credentials/{credential_id}", status_code=204)
def revoke_agent_credential(
    ...
    item = session.scalar(select(IntegrationCredentialORM).where(
        IntegrationCredentialORM.id == credential_id,
        IntegrationCredentialORM.workspace_id == workspace_id,
    ))
    if item is None:
    ...
    session.add(GovernanceAuditEventORM(
        workspace_id=workspace_id, actor_id=user.get("sub", "unknown"),
        event_type="integration.agent_credential.revoked", entity_type="integration_credential",
        entity_id=item.id, trace_id=get_trace_id(request), payload={"name": item.name},
    ))
    session.commit()
...
async def github_webhook(
    ...
    if x_github_event == "pull_request" and repo:
        ...
        pr.head_sha = (data.get("head") or {}).get("sha", pr.head_sha)
        ...
        if payload.get("action") in {"opened", "reopened", "synchronize", "ready_for_review"}:
            if repo.installation_id and AppSettings().github_app_configured:
                try:
                    paths = GitHubChecksClient().pull_request_files(repo.full_name, number, repo.installation_id)
                    existing_files = session.scalar(select(SourceSignalORM).where(
                        SourceSignalORM.pull_request_id == pr.id,
                        SourceSignalORM.signal_type == "github_pr_files",
                        SourceSignalORM.evidence_ref == f"github-files:{pr.head_sha}",
                    ))
                    if existing_files is None:
                        session.add(SourceSignalORM(workspace_id=x_workspace_id, pull_request_id=pr.id,
                            signal_type="github_pr_files", confidence=1.0,
                            evidence_ref=f"github-files:{pr.head_sha}",
                            payload={"head_sha": pr.head_sha, "changed_files": paths}))
                except httpx.HTTPError as exc:
                    session.add(GovernanceAuditEventORM(workspace_id=x_workspace_id,
                        event_type="integration.github.files_failed", entity_type="pull_request", entity_id=pr.id,
                        trace_id=get_trace_id(request), payload={"error": type(exc).__name__, "head_sha": pr.head_sha}))
            auto_evaluate_pr = pr
    elif x_github_event in {"check_run", "workflow_run"} and repo:
        data = payload.get(x_github_event) or {}
        if x_github_event == "check_run" and data.get("name") in {"GovernForge Policy Gate", "GovernForge Policy Gate"}:
            delivery.status = "processed"; session.commit()
            return {"status": "processed", "delivery_id": x_github_delivery, "ignored": "policy_gate_check"}
        ...
        run.head_sha = head_sha or None
        if pr and run.status == "completed": auto_evaluate_pr = pr
    ...
    session.flush()
    if auto_evaluate_pr is not None:
        evaluate_pull_request_record(session, x_workspace_id, auto_evaluate_pr, get_trace_id(request))
    session.commit()
